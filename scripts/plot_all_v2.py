...
def init():
    ...
    #accx_plot.set_xlabel('Time (s)')
    
    #accy_plot = plot_figure.add_subplot(3, 3, 2, sharex= accx_plot, sharey=accx_plot)
    #accy_plot.set_title('Acceleration Y')
    #accy_plot.set_xlabel('Time (s)')
    
    angz_plot = plot_figure.add_subplot(2, 2, 4, sharex= accx_plot)
    angz_plot.set_title('Pitch (Angular Velocity Y - IMU)')
    angz_plot.set_xlabel('Time (s)')

    lx_plot = plot_figure.add_subplot(2, 2, 2, sharex= accx_plot)
    lx_plot.set_title('Linear Velocity (Joystick)')
    lx_plot.set_xlabel('Time (s)')
    
    az_plot = plot_figure.add_subplot(2 ,2, 3, sharex= accx_plot, sharey=lx_plot)
    az_plot.set_title('Angular Velocity (Joystick)')
    az_plot.set_xlabel('Time (s)')

    pose_plot = plot_figure.add_subplot(2, 2, 1)
    pose_plot.set_title('Robot Position')
    red_patch = mpatches.Patch(color='b', label='Non assisted')
    red_patch2 = mpatches.Patch(color='r', label='Assisted')
    #red_patch3 = mpatches.Patch(color='g', label='Park')
    pose_plot.legend(handles=[red_patch, red_patch2])
    pose_plot.set_xlabel('X (m)')
    pose_plot.set_ylabel('Y (m)')

    adjustprops = dict(left=0.03, bottom=0.08, right=0.99, top=0.97, wspace=0.19, hspace=0.50)
                ...
                for topic, msg, t in bag.read_messages(topics=['/imu/data', '/jaguar_velocity_controller/cmd_vel', '/amcl_pose']):
                    if topic == '/imu/data':
                        if tick == 0:
                            ...
                            angz.append(msg.angular_velocity.y)
